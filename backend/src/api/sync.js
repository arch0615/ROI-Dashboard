...
router.post('/google-ads-placements/:account_id', async (req, res) => {
  const { date_preset, from, to } = req.body || {};
  try {
    const result = await withSyncLog({
      userId: req.user.id,
      source: 'google-ads-placements',
      fn: async () => {
        const r = await syncAdsPlacementsForAccount({
          userId: req.user.id,
          accountId: req.params.account_id,
          datePreset: date_preset,
          from,
...
router.post('/gam-utm-placement/:account_id', async (req, res) => {
  const { date_preset, from, to } = req.body || {};
  try {
    const result = await withSyncLog({
      userId: req.user.id,
      source: 'gam-utm-placement',
      fn: async () => {
        const r = await syncGamUtmPlacement({
          userId: req.user.id,
          accountId: req.params.account_id,
          datePreset: date_preset,
          from,
